...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var azioni = document.getElementById('Table');
    // console.log(azioni);
    ...
    azioni.addEventListener('click', function(ev) {
        ...
        if (a.attributes.hasOwnProperty('title')) {
            ...
            var idElemento = tr.childNodes[1].innerText;
        // var urlMethod = a.attributes.title.value;
        //if (a.hasAttribute('_method')) urlMethod = a.attributes._method.value;
            var urlAction = location.pathname+"/"+idElemento+"/"+actTo2;
        // console.dir(tr, idElemento);
        ...
        if (actTo2 === 'delete') {
            ...
            let JSalert = new Promise(function (resolve, reject) {
                swal({
                        title: "CANCELLAZIONE!",
                        text: "Intendi eseguire la cancellazione dell\'elemento "+idElemento+"?",
                        type: "warning",
                        showCancelButton: true,
                        //confirmButtonColor: "#DD6B55",
                        confirmButtonColor: "purple",
                        confirmButtonText: "SI, rimuovi elemento!",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            swal("Fatto!", "Elemento rimosso definitivamente!", "success");
                            eseguiCancellazione()
                        } else {
                            swal("No action!", "L\'elemento non è stato rimosso!", "error");
                            return false;
                        }
                    });
            });
