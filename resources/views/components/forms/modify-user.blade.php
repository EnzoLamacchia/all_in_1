...
{{--            {{$id=$user['id']}};--}}
            ...
            <form class="bg-white p-3 rounded-lg shadow-lg w-full" method="POST"
                  action={{ route('aggiornautente', ['id' => $user['id']]) }}>
                @csrf
                @method('PATCH')
                ...
                <div class="flex flex-row">
                    <div class="w-1/2">
                        <label class="text-gray-800 font-semibold block my-1 text-md text-left pt-3"
                               for="username">Nome</label>
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               type="text" name="name" id="name" placeholder="nome" value="{{$user['name']}}"/>
                        @error('name')
                    ...
                    <div class="w-1/2 pl-2">
                        <label class="text-gray-800 font-semibold block my-1 text-md text-left pt-3"
                               for="username">Cognome</label>
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               type="text" name="surname" id="surname" placeholder="cognome"
                               value="{{$user['surname']}}"/>
                        @error('surname')
                        <div class="text-red-500 text-left text-sm">{{ $message }}</div>
                        @enderror
                ...
                <div class="flex flex-row">
                    ...
                    <div class="w-1/2 pl-2">
                        ...
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               type="text" name="email" id="email" placeholder="@email" value="{{$user['email']}}"/>
                        @error('email')
                        <div class="text-red-500 text-left text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="flex flex-row">
                    <div class="w-1/2">
                        <label class="text-gray-800 font-semibold block my-1 text-md text-left pt-3"
                               for="user_status">Stato</label>
                        <div class="rounded-lg bg-gray-100 px-3">
                            <select class="w-full bg-gray-100 px-1 py-2 rounded-lg focus:outline-none"
                                    id="user_status" name="user_status" type="text">
                                @foreach($stati as $item)
                                    <option
                                        value="{{$item['id']}}" {{$item['user_status']==$user->UserStatus ? 'selected' : ''}}>{{$item['user_status']}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <x-sidebar.menuDxUser menDx="modificaUser" userid="{{$user->id}}"></x-sidebar.menuDxUser>
    </div>
</main>
