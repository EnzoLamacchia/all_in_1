    ...
    <div class="flex flex-row">
        <div class="w-10/12">
            <form class="bg-white p-3 rounded-lg shadow-lg w-full" method="POST"
                  action="{{route('aggiornapermesso', ['id' => $permesso['id']])}}">
                @csrf
                @method('PATCH')
                <input type="hidden" name="permission_id" value={{$permesso['id']}}>
                <div class="flex flex-row">
                    <div class="w-1/2">
                        ...
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               type="text" name="name" id="name" placeholder="denominazione permesso"
                               value="{{$permesso['name']}}"/>
                        @error('name')
                    ...
                    <div class="w-1/2 pl-2">
                        ...
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               type="text" name="guard_name" id="guard_name" placeholder="guard_name"
                               value="{{$permesso['guard_name']}}" disabled/>
                        @error('guard_name')
                        <div class="text-red-500 text-left text-sm">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <x-sidebar.menuDxPermission menDx="modificaPermesso" permissionid="{{$permesso->id}}"></x-sidebar.menuDxPermission>
    </div>
</main>
