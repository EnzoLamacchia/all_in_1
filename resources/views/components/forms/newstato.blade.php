<main class="p-2 text-center">
    <div>
        <form class="bg-white p-3 rounded-lg shadow-lg w-full" method="POST" action="{{route('salvastato')}}">
            @csrf
            <div class="flex flex-row">
                <div class="w-1/2">
                    <label class="text-gray-800 font-semibold block my-1 text-md text-left pt-3"
                           for="name">Denominazione Stato Utente</label>
                    <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                           type="text" name="name" id="name" placeholder="denominazione stato utente" value="{{old('name')}}"/>
                    @error('name')
