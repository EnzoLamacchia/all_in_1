<main class="p-2 text-center">
    <div>
        <form class="bg-white p-3 rounded-lg shadow-lg w-full" method="POST" action="{{route('salvavoce')}}">
            @csrf
            <input type="hidden" name="vocabulary_id" value={{$vocabolario['id']}}>
            <div class="flex flex-row">
                <div class="w-1/2">
                    ...
                    <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                           type="text" name="vocabularyname" id="vocabularyname" value="{{$vocabolario->name}}" disabled />
                </div>
            ...
            <div class="flex flex-row">
                <div class="w-1/2">
                    <label class="text-gray-800 font-semibold block my-1 text-md text-left pt-3"
                           for="name">Voce</label>
                    <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                           type="text" name="name" id="name" placeholder="denominazione voce di vocabolario" value="{{old('name')}}"/>
                    @error('name')
                ...
                <div class="w-1/2 pl-2">
                    ...
                    <div class="rounded-lg bg-gray-100 px-3">
                        <select class="w-full bg-gray-100 px-1 py-2 rounded-lg focus:outline-none"
                            ...
                            @foreach($vocabolario->voicesnotchildren as $item)
