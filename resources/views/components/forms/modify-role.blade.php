    ...
    <div class="flex flex-row">
        <div class="w-10/12">
            <form class="bg-white p-3 rounded-lg shadow-lg w-full" method="POST"
                  action="{{route('aggiornaruolo', ['id' => $ruolo['id']])}}">
                @csrf
                @method('PATCH')
                <input type="hidden" name="role_id" value={{$ruolo['id']}}>
                <div class="flex flex-row">
                    <div class="w-1/2">
                    ...
                    <div class="w-1/2 pl-2">
                        ...
                        <input class="w-full bg-gray-100 px-4 py-2 rounded-lg focus:outline-none"
                               ...
                               value="{{$ruolo['guard_name']}}" disabled/>
                        @error('guard_name')
                        <div class="text-red-500 text-left text-sm">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <x-sidebar.menuDxRole menDx="modificaRuolo" roleid="{{$ruolo->id}}"></x-sidebar.menuDxRole>
    </div>
</main>
