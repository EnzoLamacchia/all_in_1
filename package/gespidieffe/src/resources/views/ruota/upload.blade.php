...
<div class="flex flex-col w-full min-h-screen bg-gray-50">
    ...
    <div class="px-8 pt-8 pb-4 text-center">
        <div class="flex items-center justify-center gap-3 mb-1">
            ...
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Ruota pagine</h1>
        </div>
        <p class="text-sm text-gray-500 mt-1">Ruota singole pagine o l'intero documento</p>
    </div>
    ...
    <div class="flex flex-1 items-center justify-center px-4 py-6">
        <div class="w-full max-w-xl">
            ...
            <div class="bg-white rounded-2xl shadow-md p-8">
                <form method="POST"
                      action="{{ route('gespidieffe.ruota.upload') }}"
                    ...
                    <div id="dropZone"
                         class="relative flex flex-col items-center justify-center
                                ...
                                bg-gray-50 hover:bg-green-50 hover:border-green-400
                                transition-colors duration-200 p-10 text-center">

                        <svg class="w-14 h-14 text-green-400 mb-4" fill="none" viewBox="0 0 24 24"
                    ...
                    <div id="fileInfo" class="hidden mt-4 flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg px-4 py-3">
            ...
            <div class="mt-8 grid grid-cols-2 gap-4 text-sm text-gray-600">
                <div class="flex gap-3 items-start bg-white rounded-xl p-4 shadow-sm">
                    ...
                    <div>
                        <p class="font-semibold text-gray-800">Pagine singole</p>
                        <p class="text-xs text-gray-500 mt-0.5">Clicca ogni miniatura per ruotarla di 90° alla volta</p>
                    </div>
                </div>
                <div class="flex gap-3 items-start bg-white rounded-xl p-4 shadow-sm">
                    <span class="text-2xl">📄</span>
                    <div>
                        <p class="font-semibold text-gray-800">Tutto il documento</p>
                        <p class="text-xs text-gray-500 mt-0.5">Ruota tutte le pagine con un clic</p>
                    </div>
                </div>
...
(function () {
    ...
    dropZone.addEventListener('dragover', e => { e.preventDefault(); dropZone.classList.add('bg-green-50', 'border-green-400'); });
    dropZone.addEventListener('dragleave', () => { dropZone.classList.remove('bg-green-50', 'border-green-400'); });
    dropZone.addEventListener('drop', e => {
        e.preventDefault(); dropZone.classList.remove('bg-green-50', 'border-green-400');
